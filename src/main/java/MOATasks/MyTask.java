...
//                coleccionSeries.getSeries(datasetActual.relationName()).add(instancia.value(0), instancia.value(1));
...
//                inspeccionarConDetectorIndividualmente(instancia);
...
//        ApplicationFrame grafica = new ApplicationFrame("Grafica");

/*
        JFreeChart chart = ChartFactory.createScatterPlot("Detector de anomalías",
                "Y",
                "X",
                coleccionSeries,
...
//        chart.getRenderingHints().put(KEY_ANTIALIASING, VALUE_ANTIALIAS_ON);
...
//        grafica.setVisible(true);
    ...
    public void inspeccionarConDetectorIndividualmente(Instance instancia) {
        ...
        if (arrayDetectores.size() >= 1) {
            ...
            // Obtenemos todas las clases a tratar para este detector
//            for(Instances cod : arrayDetectores.keySet()){
...
//            coleccionSeries.getSeries(instancia.stringValue(dInstancia.classAttribute())).add(dInstancia.value(1), dInstancia.value(0));
...
//            }
    ...
    /**
     * Éste método permite normalizar el conjunto de datos, de esta forma podemos aplicar escalado
